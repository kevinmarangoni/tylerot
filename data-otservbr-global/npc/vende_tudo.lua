local internalNpcName = "Vende Tudo"
...
local itemsTable = {
	...
	["equipment"] = {
		...
		{ itemName = "might ring", clientId = 3048, buy = 8000 },
	},
	["distance"] = {
	...
	["supplies"] = {
		...
		{ itemName = "shapeshifter ring", clientId = 907, buy = 5500, count = 15 },
		{ itemName = "blueberry cupcake", clientId = 28484, buy = 50 },
		{ itemName = "strawberry cupcake", clientId = 28485, buy = 50 },
		{ itemName = "lemon cupcake", clientId = 28486, buy = 50 },
	},
	["tools"] = {
	...
	if MsgContains(message, "first rod") or MsgContains(message, "first wand") then
		if player:isMage() then
			if player:getStorageValue(Storage.FirstMageWeapon) == -1 then
				npcHandler:say("Então você quer uma {" .. ItemType(itemId):getName() .. "} para começar sua aventura?", npc, creature)
				npcHandler:setTopic(playerId, 1)
			else
				npcHandler:say("O quê? Eu já te dei uma {" .. ItemType(itemId):getName() .. "}!", npc, creature)
			end
		else
			npcHandler:say("Desculpe, você não é um druida nem um sorcerer.", npc, creature)
		end
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			player:addItem(itemId, 1)
			npcHandler:say("Aqui está, jovem adepto. Cuide-se bem!", npc, creature)
			player:setStorageValue(Storage.FirstMageWeapon, 1)
		end
		npcHandler:setTopic(playerId, 0)
	elseif MsgContains(message, "no") and npcHandler:getTopic(playerId) == 1 then
		npcHandler:say("Ok então.", npc, creature)
		npcHandler:setTopic(playerId, 0)
	elseif categoryTable then
		local remainingCategories = npc:getRemainingShopCategories(message:lower(), itemsTable)
		npcHandler:say("Claro! Aqui estão meus produtos. Você também pode ver " .. remainingCategories .. ".", npc, player)
...
npcHandler:setMessage(
	MESSAGE_GREET,
	"Bem-vindo, |PLAYERNAME|! \z
	Eu sou o Vende Tudo e tenho uma ENORME variedade de itens para você! \z
	Posso te ajudar com {various}, {exercise weapons}, {equipment}, {distance}, {rods}, {wands}, {potions}, {runes}, {supplies}, {tools} e {postal}! \z
	Basta me pedir uma {trade} ou falar o nome da categoria que deseja ver!"
)
npcHandler:setMessage(MESSAGE_SENDTRADE, "Claro! Aqui está minha loja completa. Você também pode pedir para ver apenas uma categoria específica como {various}, {exercise weapons}, {equipment}, {distance}, {rods}, {wands}, {potions}, {runes}, {supplies}, {tools} ou {postal}!")
npcHandler:setMessage(MESSAGE_FAREWELL, "Até logo! Volte sempre!")
npcHandler:setMessage(MESSAGE_WALKAWAY, "Até logo! Volte sempre!")
